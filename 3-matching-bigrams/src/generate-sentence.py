...
def match_sentence_bigrams(img_dir, font_name, sentence):
    ...
    for n in range(len(sentence)-2):
        bi1 = str(sentence[n:n+2])
        bi2 = str(sentence[n+1:n+3])
        trigram = bi1 + bi2[1]

        if n == 0:
            img1 = Image.open(img_dir + str(q) + '.jpg').convert('L')
        ...
        img2 = Image.open(img_dir + str(w) + '.jpg').convert('L')

        print('trigram: {0}'.format(trigram))

        history_path = '../data/state-histories/{0}/{1}/{2}.json'.format('urdu1', font_name, str(z))
        #history_path = '../data/state-histories/{0}/{1}/{2}.json'.format('urdu1', font_name,trigram.replace(' ', '-'))
    ...
    composite_img.save('../../visualisations/sentences/{0}-{1}.jpg'.format('urdu6', font_name), quality=100)


if __name__ == "__main__":
    font_idxs = range(1, 14)

    sen = list()
    with codecs.open('sentence2.txt', 'r', encoding="utf-8") as f:
        for line in f:
            line = u"%s" % line
            # char = line.split()[0]
    ...
    for font_idx in font_idxs:
        ...
        img_dir = '../../visualisations/sentence-bigrams/{0}/{1}/'.format('urdu1', font_name)
